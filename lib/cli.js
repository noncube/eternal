...
nomnom.command('start')
    ...
    .option('key', {
        help: 'String to use to identify the app. RECOMMENDED'
    })
    .option('log', {
        help: 'Explicit path to logfile [~/.eternal/<KEY>.log]'
    })
    .option('watch', {
        abbr: 'w',
        flag: true,
        ...
        if (opts.json) {
            ...
            var options = {
                ...
                columns: [
                    {field: "pid", name: "PID"},
                    {field: "key",  name: "Key"},
                    {field: "file", name: "Command"},
                    {field: "max",  name: "Max"},
                    {field: "current", name: "Current"},
                    {field: "log", name: "Logfile"},
                    {field: "started", name: "Started At"}
                ]
            };

            data = data.map(function(item) {
                item.file = item.file || item.exec;
                item.started = new Date(item.started);
                return item;
            });

            output = asciitable(options, data);
        }
        console.log(output);
    });
}
